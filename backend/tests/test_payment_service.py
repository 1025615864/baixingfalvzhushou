...
async def _ensure_test_user(db: AsyncSession, user_id: int = 1) -> User:
    """确保测试用户存在"""
...
class TestPaymentConcurrency:
    """支付并发测试类（跳过：SQLite 不支持并发 session）"""

    @pytest.mark.skip(reason="SQLite does not support concurrent session operations")
    @pytest.mark.asyncio
