...
| 指标 | 目标 | 工具 |
|------|------|------|
| API P95响应时间 | < 500ms | k6压测脚本(k6-load-test.js) |
| 首页加载时间 | < 2s | Lighthouse CI(lighthouserc.json) |
| 数据库慢查询 | < 100ms | 慢查询日志 |
| 并发用户支持 | 500+ | k6加压(10→100→500 VU) |

#### 任务 4.2: 安全加固 ✅

| 子任务 | 说明 | 状态 |
|--------|------|------|
| 4.2.1 | OpenAPI敏感字段标记writeOnly(19个字段) | ✅ 已完成 |
| 4.2.2 | 限流降级策略（Redis不可用时内存限流） | ✅ 已验证可用 |
| 4.2.3 | 清理测试中硬编码密码(8文件96处) | ✅ 已完成 |
| 4.2.4 | 生产环境禁用openapi.json端点 | ✅ 已完成 |

#### 任务 4.3: 上线检查 ✅

| 子任务 | 脚本 | 状态 |
|--------|------|------|
| 4.3.1 | 健康检查脚本(14服务42端点) | ✅ health-check.sh |
| 4.3.2 | Docker构建验证脚本 | ✅ verify-docker-builds.sh |
| 4.3.3 | 生产就绪检查(8项) | ✅ production-readiness.sh |
...
|------|------|--------|---------|------|
| G1 | 修复前端全部TS类型错误 | P0 | `tsc --noEmit` 零错误 | ✅ 已完成 |
| G2 | 补齐微服务核心功能 | P0 | 5个低完成度服务达到60%+ | ✅ 已完成(6个达65%+) |
| G3 | 统一认证体系 | P0 | 全服务使用JWTKeyManager + RS256 | ✅ 已完成(硬编码移除) |
| G4 | 补齐微服务测试 | P1 | 每个服务至少10个测试用例 | ✅ 已完成(117用例) |
| G5 | 补齐基础设施配置 | P1 | 所有服务有Dockerfile + .env.example | ✅ 已完成(14/14) |
| G6 | BFF层瘦身 | P1 | main.py < 400行 | ✅ 已完成(305行) |
| G7 | 前端E2E测试覆盖 | P2 | 核心流程E2E测试通过 | ✅ 已完成(16spec) |
| G8 | 性能基准建立 | P2 | API P95 < 500ms，首页加载 < 2s | ✅ 已完成(k6+Lighthouse) |
