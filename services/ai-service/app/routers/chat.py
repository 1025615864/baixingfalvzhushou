...
from app.dependencies.auth import get_required_user, UserContext
...
async def chat(request: ChatRequest, user: UserContext = Depends(get_required_user)):
    """AI对话（非流式）"""
...
async def chat_stream(request: ChatRequest, user: UserContext = Depends(get_required_user)):
    """AI对话（流式SSE）"""
    sanitize_result = sanitize_user_input(request.message)
    if not sanitize_result.success:
...
async def get_history(session_id: str, user: UserContext = Depends(get_required_user)):
    """获取会话历史"""
    try:
        orchestrator = get_chat_orchestrator()
