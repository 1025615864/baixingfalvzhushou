"""知识库向量库 - 支持本地Embedding和远程Embedding服务"""
import os
import logging
...
async def search_knowledge_async(query: str, top_k: int = 5) -> list[dict]:
    """异步检索知识库"""
    return await knowledge_vector_store.search_async(query=query, top_k=top_k)


def search_knowledge(query: str, top_k: int = 5) -> list[dict]:
    """检索知识库"""
    return knowledge_vector_store.search(query=query, top_k=top_k)


async def get_collection_count_async() -> int:
    """异步获取知识库文档数量"""
    return await knowledge_vector_store.count_async()


def get_collection_count() -> int:
    """获取知识库文档数量"""
    return knowledge_vector_store.count()
