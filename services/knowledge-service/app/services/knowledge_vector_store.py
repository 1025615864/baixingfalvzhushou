"""知识库向量存储模块 - 支持本地Embedding和远程Embedding服务"""
import os
import logging
...
def get_pgvector_store():
    ...
    try:
        ...
        if not database_url:
            logger.warning("KNOWLEDGE_DATABASE_URL not set, using ChromaDB")
            return None
        ...
        logger.info("pgvector store initialized for knowledge service")
        return _pgvector_store
    except Exception as e:
...
async def search_knowledge_async(query: str, top_k: int = 5) -> list[dict]:
    """异步搜索知识库 - 自动选择pgvector或ChromaDB"""
...
async def add_knowledge_async(documents: list[dict], ids: list[str]):
    """异步添加知识文档"""
    store_type = get_vector_store_type()

    if store_type == "pgvector":
        await _add_knowledge_pgvector_async(documents, ids)
    else:
        await _add_knowledge_chroma_async(documents, ids)


def add_knowledge(documents: list[dict], ids: list[str]):
    """同步添加知识文档（使用本地embedding）"""
    store_type = get_vector_store_type()

    if store_type == "pgvector":
        _add_knowledge_pgvector_sync(documents, ids)
    else:
        _add_knowledge_chroma_sync(documents, ids)


async def _add_knowledge_pgvector_async(documents: list[dict], ids: list[str]):
    """使用pgvector异步添加文档"""
    store = get_pgvector_store()
    if not store:
        logger.warning("pgvector not available, falling back to ChromaDB")
...
def _add_knowledge_pgvector_sync(documents: list[dict], ids: list[str]):
    """使用pgvector同步添加文档"""
    store = get_pgvector_store()
    if not store:
        logger.warning("pgvector not available, falling back to ChromaDB")
...
async def _add_knowledge_chroma_async(documents: list[dict], ids: list[str]):
    try:
        collection = get_chroma_collection()
        if collection is None:
...
def _add_knowledge_chroma_sync(documents: list[dict], ids: list[str]):
    try:
        collection = get_chroma_collection()
        if collection is None:
            logger.warning("ChromaDB not available, cannot add documents")
...
async def delete_knowledge_async(ids: list[str]):
    """异步删除知识文档"""
    store_type = get_vector_store_type()

    if store_type == "pgvector":
        await _delete_knowledge_pgvector_async(ids)
    else:
        _delete_knowledge_chroma_sync(ids)


def delete_knowledge(ids: list[str]):
    """同步删除知识文档"""
    store_type = get_vector_store_type()

    if store_type == "pgvector":
        _delete_knowledge_pgvector_sync(ids)
    else:
        _delete_knowledge_chroma_sync(ids)


async def _delete_knowledge_pgvector_async(ids: list[str]):
    """使用pgvector异步删除"""
    store = get_pgvector_store()
    if not store:
        logger.warning("pgvector not available, falling back to ChromaDB")
...
def _delete_knowledge_pgvector_sync(ids: list[str]):
    """使用pgvector同步删除"""
    store = get_pgvector_store()
    if not store:
        logger.warning("pgvector not available, falling back to ChromaDB")
...
def _delete_knowledge_chroma_sync(ids: list[str]):
    try:
        collection = get_chroma_collection()
        if collection is None:
...
async def get_collection_count_async() -> int:
    ...
    try:
        ...
        logger.error("Failed to count knowledge collection")
        return 0
...
def get_collection_count() -> int:
    ...
    try:
        ...
        logger.error("Failed to count knowledge collection (sync)")
        return 0


def get_all_knowledge_ids() -> list[str]:
    store_type = get_vector_store_type()

    if store_type == "pgvector":
        logger.info("pgvector get_all_ids not implemented yet")
        return []
    ...
    try:
        ...
        logger.error("Failed to get all knowledge IDs")
        return []


async def get_knowledge_async(query: str, top_k: int = 5) -> list[dict]:
    return await search_knowledge_async(query, top_k)
