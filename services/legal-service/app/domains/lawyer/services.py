...
class LawyerDomainService:
    ...
    async def get_lawyer(self, lawyer_id: int) -> Optional[Lawyer]:
        if self.cache:
            cached = await self.cache.get_lawyer_detail(lawyer_id)
            if cached:
                return cached
        ...
        if lawyer and self.cache:
            await self.cache.set_lawyer_detail(
                lawyer_id,
                {
                    "id": lawyer.id,
                    "name": lawyer.name,
    ...
    async def list_lawyers(
        ...
        if self.cache:
            cached = await self.cache.get_lawyer_list(specialty, page)
            if cached:
                return cached.get("lawyers", []), cached.get("total", 0)
        ...
        if self.cache:
            await self.cache.set_lawyer_list(
                specialty, page,
                {"lawyers": list(lawyers), "total": total}
            )
