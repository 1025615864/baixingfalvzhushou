...
class ConsultationService:
    ...
    async def complete(
        ...
        if not ConsultationStateMachine.can_transition_from(consultation.status, ConsultationStatus.ANSWERED.value):
            raise TransitionError(
                current_state=consultation.status,
    ...
    async def cancel(
        ...
        if not ConsultationStateMachine.can_transition_from(consultation.status, ConsultationStatus.CANCELLED.value):
            raise TransitionError(
                current_state=consultation.status,
    ...
    async def get_stats(self, lawyer_id: Optional[int] = None) -> dict:
        ...
        pending_result = await self.db.execute(
            select(func.count()).select_from(query.where(Consultation.status == "pending").subquery())
        )
        pending = pending_result.scalar() or 0

        processing_result = await self.db.execute(
            select(func.count()).select_from(query.where(Consultation.status == "processing").subquery())
        )
        processing = processing_result.scalar() or 0

        answered_result = await self.db.execute(
            select(func.count()).select_from(query.where(Consultation.status == "answered").subquery())
        )
