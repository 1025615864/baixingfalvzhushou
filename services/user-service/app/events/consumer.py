"""Kafka Consumer - 消费事件（User Service）

User Service 主要作为事件生产者，但也会消费部分事件：
- user.account.deleted: 清理本地 Token 黑名单
- user.role.changed: 更新本地用户缓存权限
"""
import os
import json
import logging
...
class UserEventConsumer:
    ...
    def __init__(self):
        ...
        self.group_id = os.getenv("KAFKA_CONSUMER_GROUP_ID", "user-service-internal")
    ...
    async def close(self):
        logger.info("Initiating user service consumer graceful shutdown...")
        ...
        if self._consumer:
            ...
            self._consumer = None
        logger.info("User service consumer shutdown complete")
    ...
    async def start(self):
        ...
        self._running = True
        self._graceful_shutdown = False
        logger.info("User service event consumer starting...")
...
async def handle_user_deleted(event: Dict[str, Any]):
    ...
    try:
        from app.services.token_service import token_manager
        await token_manager.revoke_all_user_tokens(int(user_id))
        logger.info(f"Cleaned up tokens for deleted user: {user_id}")
    except Exception as e:
        logger.error(f"Failed to cleanup tokens for user {user_id}: {e}")


async def handle_role_changed(event: Dict[str, Any]):
    """处理角色变更事件 - 可用于记录审计或更新缓存"""
