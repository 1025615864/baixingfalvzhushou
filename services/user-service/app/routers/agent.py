...
router = APIRouter(dependencies=[Depends(require_domain_role("global"))])
...
@router.post("/churn-predict", response_model=AgentResponse)
async def churn_predict(req: AgentRequest):
    """流失预测"""
    return AgentResponse(result="流失预测完成", suggestions=["关注高风险用户"])
